...
- Verify actual final outcomes independently. A DONE choice is not proof of success; callers can pass a verify spec ({text, url_contains}) that code checks on every observation.
- Jev's done/risk nouls and choice confidence feed the deterministic gate in policy.py (done/confirm/escalate/stop). Held decisions stay unconsumed until approve=true or a fresh predict.
